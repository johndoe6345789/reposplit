...
**Resolved 2026-08-11:**
- `frontends/workflowui` → is now `AutoMetabuilder`'s app (confirmed: workflow UI *is* the same product as AutoMetabuilder's AI-powered SDLC workflow tool, not a mismatch after all). **Two corrections along the way:** (1) the first attempt overlay-copied workflowui straight onto the repo root, colliding with the existing `backend/` + `frontend/` app — two parallel apps in one repo. Reverted with `git revert` and redid it as a `workflowui/` subfolder. (2) That still left two copies of the app (old `backend/`+`frontend/` alongside the new `workflowui/`) — only one should exist, and workflowui is the newer version of the same product, so the old `backend/`+`frontend/` was removed and `workflowui/`'s contents promoted to the repo root as the single app. Checked the other 5 merged pairs for the original collision issue — they were fine because source and destination already shared the same root layout.
